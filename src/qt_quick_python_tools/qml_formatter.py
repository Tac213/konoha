...
def format_files(file_paths, dont_delete_backup_file) -> int:
    ...
    for file_path in file_paths:
        ...
        logging.info('Running command "%s" in "%s"', " ".join(command_args), deploy_env.root_path)
        with subprocess.Popen(command_args, cwd=deploy_env.root_path) as p:
            p.wait()
        if p.returncode:
            return p.returncode
...
def main(args) -> int:
    ...
    if args.all:
        file_paths = []
        for root, _, file_names in os.walk(deploy_utils.get_app_project_view_directory()):
